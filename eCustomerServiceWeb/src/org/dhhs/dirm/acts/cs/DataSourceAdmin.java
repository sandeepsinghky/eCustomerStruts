...
/**
 * Hit JDBC's Implementation of Database Connection Pooling This Class is
 * configured to create a pool of datasource objects so that the servlets can
 * share datasource objects from the pool.
 * <p>
...
public class DataSourceAdmin
{
	...
	/**
	 * Constructor for the Pooled DataSource. This method obtains all the
	 * properties from the properties file placed in the classpath of the
	 * servlets. Make sure to modify the properties file if any changes are
	...
	public DataSourceAdmin()
	{
		...
		try
		{
			...
			// Using a connection pooling data source actually requires two data
			// sources:
			// one that implements DataSource and the other that implements
			// Db2PooleConnectionDataSource which is connection
			// (PooledConnection) factory.
			// The DataSource's implementation uses Db2PooledDataSource as a
			// connection
			// factory while it manages the pool of connections.
			// An administrative application like this will create both type of
			// classes
			// but a user application only use the DataSource implementation and
			// does
			// not care and doesn't know anything about the
			// Db2PooleConnectionDataSource
			// behind the scene.

			// Create and bind a Db2PooledDataSource and a
			// Db2ConnectionPoolDataSource.
			// A Db2PooledDataSource is a DataSource that implements connection
			// pooling management.
			// A Db2PooledDataSource does not create the actual physical
			// connections,
			// rather it relies on Db2ConnectionPoolDataSource as the connection
			// factory.

			// Create new instance of a pool connection factory
			// Db2ConnectionPoolDataSource
			hit.db2.Db2ConnectionPoolDataSource cpds = new hit.db2.Db2ConnectionPoolDataSource();

			// Set connection properties for the data source (connection
			// factory)
			...
			connectionOptions += "auto_type_conversion=yes;remove_trailing_blanks=yes;remove_sql_white_spaces=yes;application_server=WAS;";

			// 8/11/03 cpds.setConnectionOptions("package_collection_id=" +
			// collectionID + ";commit_select=" + commitSelect);
			...
			// This name will be used as data source for a Db2PooledDataSource
			// later
			// This data source is not meant to be used by an end-user
			// application
			try
			{
			...
			// Create new instance of Db2PooledDataSource which implements
