...
public class PasswordResetAction extends DispatchAction
{
	...
	private final static Logger log = Logger.getLogger(PasswordResetAction.class);

	/**
	 * @see org.apache.struts.action.Action#authenticate(ActionMapping,
	 *      ActionForm, HttpServletRequest, HttpServletResponse)
	 */
	public ActionForward resetPassword(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException
	{
		...
		try
		{
			...
			String newPassword = ((LogonForm) form).getNewPassword();
			String confirmPassword = ((LogonForm) form).getConfirmPassword();
			...
			try
			{
				...
				/**
				 * The user was presented the Logon.jsp page with new password
				 * and confirm password fields update the ejb with the new
				 * password and set the status to active.
				 */

				if (newPassword != null && confirmPassword != null)
				{

					log.debug("User completed password reset");
					getStats = true;

					userEntityBean.setIdPassword(newPassword);
					userEntityBean.setIdWrkrUpdate(username);
					userEntityBean.setCdPasswordStatus("A");
					user.setUserEntityBean(userEntityBean);

					/*
					 * user.setIdPassword(newPassword);
					 * user.setIdWrkrUpdate(username);
					 * user.setCdPasswordStatus("A");
					 */
				...
				{
					log.info("User Password Reset Failed. Bad Password for " + username);
			...
			} catch (javax.ejb.FinderException fe)
			{
				...
				new ApplicationException(fe.getMessage(), fe, new ErrorDescriptor("PasswordReset", "execute"));
			...
			} catch (NamingException ne)
			{
				...
				new ApplicationException(ne.getMessage(), ne, new ErrorDescriptor("PasswordReset", "execute"));
			...
			} catch (BusinessLogicException ble)
			{
				...
				new ApplicationException(ble.getMessage(), ble, new ErrorDescriptor("PasswordReset", "execute"));
		...
		} catch (Exception e)
		{
			...
			new ApplicationException(e.getMessage(), e, new ErrorDescriptor("PasswordReset", "execute"));
